...
function averageNumbers () {
  ...
  render(`<p>Average: <b>${avg}<b></p><ul class="list-group">${list}</ul>`)
}
...
function randomBetween () {
  ...
  if (min > max){
    render('<p class="text-danger">Minimum must be less than Maximum number.</p>')
    return
  }

  const rndNum = Math.floor(Math.random()*(max - min + 1))

  render(`<p>Random number between ${min} and ${max}: <b>${rndNum}</b>`)
}
